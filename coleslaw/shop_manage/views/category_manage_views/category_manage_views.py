...
class CategoryManageView(View):
    ...
    def get(self, request: HttpRequest, *args, **kwargs):
        ...
        context['MainCategory'] = MainCategory.objects.filter(shop=shop).order_by('name_kr')
    ...
    def post(self, request: HttpRequest, *args, **kwargs):
        ...
        if categoryType == 'main':
            ...
            return JsonResponse({'message' : '생성 완료', 'id':category.id, 'type':'main', 'data':list(MainCategory.objects.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 201)
            ...
            return JsonResponse({'message' : '생성 완료', 'id':category.id, 'type':'sub', 'data':list(main_category.sub_category.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 201)
        else:
    ...
    def put(self, request: HttpRequest, *args, **kwargs):
        ...
        if categoryType == 'main':
            ...
            try:
                ...
                return JsonResponse({'message':'이미 대분류에 존재하는 카테고리 입니다.'}, status = 400)            
            return JsonResponse({'message' : '수정 완료', 'id':category.id, 'type':'main', 'data':list(MainCategory.objects.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 201)
        elif categoryType == 'sub':
            ...
            try:
                ...
                return JsonResponse({'message':'이미 소분류에 존재하는 카테고리 입니다.'}, status = 400)
            return JsonResponse({'message' : '수정 완료', 'id':category.id, 'type':'sub', 'data':list(category.main_category.sub_category.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 201)
        else:
            return JsonResponse({'message':'전달값 오류..'}, status = 400)
    ...
    def delete(self, request: HttpRequest, *args, **kwargs):
        ...
        if categoryType == 'main':
            ...
            return JsonResponse({'message' : '삭제 완료', 'data':list(MainCategory.objects.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 200)
        elif categoryType == 'sub':
            category = SubCategory.objects.get(id=categoryId, shop=shop)
            ...
            try:
                ...
                return JsonResponse({'message':'하위 노드들이 있어 삭제 불가능합니다.'}, status = 400)
            return JsonResponse({'message' : '삭제 완료', 'data':list(main_category.sub_category.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))}, status = 200)
        else:
            return JsonResponse({'message':'전달값 오류..'}, status = 400)
...
def sub_category(request: HttpRequest, *args, **kwargs):
    ...
    try:
        data = list(MainCategory.objects.get(pk=parent_id, shop=shop).sub_category.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))
...
def category(request: HttpRequest, *args, **kwargs):
    ...
    if not shop:
        return JsonResponse({'message' : '가맹점 오류'},status = 400)
    main_category = list(MainCategory.objects.filter(shop=shop).order_by('name_kr').values('id', 'name_kr', 'name_en'))
